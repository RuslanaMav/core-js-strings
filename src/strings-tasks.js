...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) !== -1) {
    const s1 = str.slice(0, str.indexOf(value));
    const s2 = str.slice(str.indexOf(value) + value.length);
    s = s1 + s2;
  }
  return s;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (str.lastIndexOf(value) !== -1) {
    const s1 = str.slice(0, str.lastIndexOf(value));
    const s2 = str.slice(str.lastIndexOf(value) + value.length);
    s = s1 + s2;
  }
  return s;
}
...
function countVowels(str) {
  const c = str.match(/[aeiouyAEIOUY]/g);
...
function isPalindrome(str) {
  const s1 = str.toLowerCase().replace(/[\s .,!?]/g, '');
...
function encodeToRot13(str) {
  ...
  const cip = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let str2 = '';
  for (let i = 0; i < str.length; i += 1) {
    const d = alp.indexOf(str[i]);
    str2 += d === -1 ? str[i] : cip[d];
  }
  return str2;
}
